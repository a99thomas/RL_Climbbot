# climb_env.py
"""
ClimbEnv using analytic R-R-P IK per arm (with numeric-Jacobian fallback).
...
# -------------------- numeric Jacobian IK utils (kept as fallback) --------------------
...
# -------------------- Analytical R-R-P IK for each arm --------------------
def analytic_ik_rrp(target_w: np.ndarray,
    ...
    """
    Analytic IK for R-R-P arm.

    CONFIG (cfg) keys expected:
      - 'shoulder_pos_b': np.array(3,) --> shoulder position in base frame (meters)
      - 'upper_arm_len': float --> fixed link length from shoulder rotation to prismatic mount (meters)
      - 'min_q': (q1_min,q2_min,q3_min) and 'max_q': likewise -> joint limits (optional; used for clipping)
      - 'prismatic_min'/'prismatic_max' optional
      - 'joint_signs' optional (dict) to flip axes if your XML uses different sign conventions:
            {'q1': 1.0, 'q2': 1.0, 'q3': 1.0}
    RETURNS:
      delta_q (length 3) in joint space (proposed change), or None if unreachable / invalid.
    NOTES / ASSUMPTIONS:
      - joint1 rotates about the base Z axis (azimuth)
      - joint2 rotates in the plane formed by the rotated X-Z after joint1 (elevation)
      - prismatic joint extends along local +X after joint2
      - target_w and base_* are in world frame (target_w is desired gripper world position)
    """
    try:
        # transform target into base-local coordinates
        target_b = world_to_local(target_w, base_pos_w, base_quat_w)  # in base frame

        shoulder_b = np.asarray(cfg.get("shoulder_pos_b", np.zeros(3)))
        v = target_b - shoulder_b  # vector from shoulder to desired gripper pos in base frame

        # quick reach check
        upper_len = float(cfg.get("upper_arm_len", 0.1))
        min_p = cfg.get("prismatic_min", -np.inf)
        max_p = cfg.get("prismatic_max", np.inf)

        # compute q1 (azimuth) as atan2 of y, x
        q1 = np.arctan2(v[1], v[0])  # rotation about base z to point toward target projection

        # rotate vector into shoulder-local plane where q1=0 (i.e., rotate -q1 about z)
        c = np.cos(-q1); s = np.sin(-q1)
        Rz = np.array([[c, -s, 0], [s, c, 0], [0, 0, 1]])
        v_local = Rz @ v  # now v_local's y should be ~0 (ideally), x forward, z up/down

        vx = v_local[0]
        vz = v_local[2]

        # planar distance in the q1-rotated XZ plane from shoulder to target projection
        r = np.hypot(vx, vz)

        # required prismatic extension to reach (simple model):
        # assume the two revolute joints set a fixed planar offset (upper_len) from shoulder to prismatic origin;
        # the prismatic adds additional reach along +x in that rotated plane.
        # So desired prismatic = r - upper_len (if positive), clamped to allowed prismatic min/max.
        desired_p = r - upper_len

        # if target is "behind" (r < some small negative), allow negative extension if prismatic allows
        desired_p_clamped = float(np.clip(desired_p, min_p, max_p))

        # compute q2 (elevation) to point the prismatic axis toward target:
        # We want the prismatic axis (which after q2 points along some direction) to line up so that
        # after extending desired_p we hit the target. Given our simple geometry, we set q2=atan2(vz, vx)
        # (i.e., angle of vector in X-Z plane).
        q2 = np.arctan2(vz, vx)  # rotation in the plane to angle towards target

        # apply joint sign conventions if provided
        signs = cfg.get("joint_signs", {"q1": 1.0, "q2": 1.0, "q3": 1.0})
        q1 *= float(signs.get("q1", 1.0))
        q2 *= float(signs.get("q2", 1.0))
        desired_p_clamped *= float(signs.get("q3", 1.0))

        # read current q for those joints from qpos and addrs
        cur_q = np.zeros(3)
        for i, addr in enumerate(qpos_addrs):
            cur_q[i] = float(qpos[addr])

        # desired absolute q vector
        desired_q = np.array([q1, q2, desired_p_clamped], dtype=float)

        # optional joint limits clipping applied here (if provided)
        qmin = cfg.get("min_q", (-np.inf, -np.inf, -np.inf))
        qmax = cfg.get("max_q", (np.inf, np.inf, np.inf))
        desired_q = np.clip(desired_q, np.array(qmin, dtype=float), np.array(qmax, dtype=float))

        delta_q = desired_q - cur_q
        return delta_q
    except Exception as e:
        # if anything goes wrong, return None to let caller fallback
        return None
...
class ClimbEnv(gym.Env):
    ...
    def __init__(self, xml_path: str,
                 ...
                 # new: arm analytic configs (editable)
                 right_arm_cfg: Optional[Dict] = None,
                 left_arm_cfg: Optional[Dict] = None):
        ...
        # Analytical arm configuration (defaults - tweak to match your XML)
        # shoulder_pos_b: shoulder location in base frame (meters)
        # upper_arm_len: planar offset from shoulder to prismatic origin
        self.right_arm_cfg = right_arm_cfg or {
            "shoulder_pos_b": np.array([0.08, -0.12, 0.0], dtype=float),
            "upper_arm_len": 0.10,
            "min_q": (-np.pi, -np.pi/2, 0.0),   # q1,q2,q3 min
            "max_q": (np.pi, np.pi/2, 0.20),   # q1,q2,q3 max
            "prismatic_min": 0.0,
            "prismatic_max": 0.20,
            "joint_signs": {"q1": 1.0, "q2": 1.0, "q3": 1.0}
        }
        self.left_arm_cfg = left_arm_cfg or {
            "shoulder_pos_b": np.array([0.08, 0.12, 0.0], dtype=float),
            "upper_arm_len": 0.10,
            "min_q": (-np.pi, -np.pi/2, 0.0),
            "max_q": (np.pi, np.pi/2, 0.20),
            "prismatic_min": 0.0,
            "prismatic_max": 0.20,
            "joint_signs": {"q1": 1.0, "q2": 1.0, "q3": 1.0}
        }
        ...
        self.reward_params = {
            ...
            "climb_scale": 0.0,
            "climb_success_delta": 0.04,
            "climb_success_bonus": 200.0,
            "ctrl_penalty_scale": 0.0
        }
    ...
    # new helper: try analytic IK, fallback numeric if necessary
    def _solve_ik_for_arm(self, target_w: np.ndarray, site_id: int,
                          qpos_addrs: List[int], qpos_indices: List[int], cfg: Dict) -> Tuple[np.ndarray, bool]:
        """
        Returns (delta_q, used_analytic_bool)
        - delta_q: array length = len(qpos_addrs)
        - used_analytic_bool: True if analytic succeeded, False if fallback used
        """
        base_pos = self.data.site_xpos[self.base_site_id].copy()
        base_quat = site_quat_from_xmat(self.data, self.base_site_id)
        # attempt analytic IK
        delta_q = analytic_ik_rrp(target_w, base_pos, base_quat, self.data.qpos, qpos_addrs, cfg)
        if delta_q is not None and delta_q.shape[0] == len(qpos_addrs):
            return delta_q, True

        # fallback numeric Jacobian (compute jacobian for this arm only)
        J = numeric_jacobian(self.model, self.data, site_id, qpos_indices, eps=self.ik_fd_eps)
        # Solve least squares for delta_q: use damped pseudoinverse
        pinv = damped_pinv(J, self.ik_damping)  # shape (n,3)
        err = (target_w - self.data.site_xpos[site_id]).reshape(3,)
        delta = (pinv @ err).squeeze()
        return delta, False
    ...
    def step(self, action: np.ndarray):
        """
        Replaces the original step(). Now uses analytic IK for RRP arms where possible.
        """
        self._step_count += 1
        ...
        # ---------------- analytic IK attempt for right arm ----------------
        delta_r, used_r_analytic = self._solve_ik_for_arm(r_des_w, self.r_grip_site_id,
                                                          self.right_qpos_addrs, self.right_qpos_indices,
                                                          self.right_arm_cfg)
        delta_l, used_l_analytic = self._solve_ik_for_arm(l_des_w, self.l_grip_site_id,
                                                          self.left_qpos_addrs, self.left_qpos_indices,
                                                          self.left_arm_cfg)

        # If analytic return lengths mismatch, zero-fill (safety)
        if delta_r is None:
            delta_r = np.zeros(len(self.right_qpos_addrs))
            used_r_analytic = False
        if delta_l is None:
            delta_l = np.zeros(len(self.left_qpos_addrs))
            used_l_analytic = False

        # -------------------- safety + smoothing pipeline (identical) --------------------
        ...
        close_thresh = self.reward_params.get("close_thresh", 0.045)
        terminated = False
        ...
        climb_delta = float(self.reward_params.get("climb_success_delta", 0.045))
        ...
        info = {
            ...
            "reward_dist": float(base_dist_reward),
            "reward_climb": float(climb_reward),
            "reward_shaping": float(0.5 * right_shaping),
            "reward_ctrl_pen": float(ctrl_pen),
            "is_success": bool(terminated),
            "reward_contact": float(contact_bonus),
            "used_r_analytic": bool(used_r_analytic),
            "used_l_analytic": bool(used_l_analytic),
        }
