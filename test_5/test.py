...
try:
    from climb_env_5 import ClimbBotEnv  # if you put env in climbbot_env.py
except Exception:
...
def main():
    ...
    workspace_min = getattr(env, "workspace_min", np.array([0.2, -0.4, -0.5], dtype=np.float32)).astype(np.float64)
    workspace_max = getattr(env, "workspace_max", np.array([0.8, 0.4, 0.2], dtype=np.float32)).astype(np.float64)
